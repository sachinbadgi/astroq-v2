...
class MockContext:
    def __init__(self, natal_pos, annual_pos, age=30):
        self.natal_pos = natal_pos
        self.annual_pos = annual_pos
        self.age = age
    def get_natal_house(self, planet):
        return self.natal_pos.get(planet)
    def get_house(self, planet):
        return self.annual_pos.get(planet)
    def is_awake(self, planet):
        return len(self.annual_pos) > 1
    def has_180_degree_block(self, planet):
        h = self.annual_pos.get(planet)
        if not h: return False
        opp = h + 6 if h <= 6 else h - 6
        return any(v == opp for v in self.annual_pos.values())
...
class TestDoubtfulNatalPromiseDetection(unittest.TestCase):
    ...
    def test_roof_and_well(self):
        ...
        natal = {"Rahu": 5, "Moon": 4, "Saturn": 10}
        ctx = MockContext(natal, {})
        promises = self.engine._identify_doubtful_natal_promises(ctx)
    ...
    def test_doubtful_venus_h4(self):
        ...
        natal = {"Venus": 4, "Saturn": 2, "Jupiter": 9}
        ctx = MockContext(natal, {})
        promises = self.engine._identify_doubtful_natal_promises(ctx)
        names = [p["name"] for p in promises]
        self.assertTrue(
    ...
    def test_houses_2_7_blank(self):
        ...
        natal = {"Sun": 1, "Mars": 3, "Jupiter": 9}  # no planets in H2 or H7
        ctx = MockContext(natal, {})
        promises = self.engine._identify_doubtful_natal_promises(ctx)
        names = [p["name"] for p in promises]
        self.assertTrue(
    ...
    def test_no_false_detection(self):
        ...
        natal = {"Sun": 1, "Moon": 2, "Venus": 7, "Jupiter": 9, "Saturn": 10}
        ctx = MockContext(natal, {})
        promises = self.engine._identify_doubtful_natal_promises(ctx)
...
class TestDoubtfulTimingResolutionAndTrigger(unittest.TestCase):
    ...
    def test_resolution_via_exaltation(self):
        ...
        annual = {"Venus": 12, "Saturn": 6, "Jupiter": 3}
        ctx = MockContext(natal, annual)

        evals = self.engine.evaluate_doubtful_timing(ctx, domain="marriage")
    ...
    def test_trigger_via_debilitation(self):
        ...
        """
        natal = {"Rahu": 5, "Moon": 4}
        annual = {"Rahu": 9, "Moon": 11}  # Rahu debilitated in H9
        ctx = MockContext(natal, annual)

        evals = self.engine.evaluate_doubtful_timing(ctx, domain="progeny")
    ...
    def test_domain_filter(self):
        ...
        natal = {"Rahu": 5, "Moon": 4, "Venus": 4}
        annual = {"Rahu": 9, "Venus": 12, "Moon": 6}
        ctx = MockContext(natal, annual)

        # Only progeny domain
        evals_progeny = self.engine.evaluate_doubtful_timing(ctx, domain="progeny")
        for e in evals_progeny:
            self.assertEqual(e["domain"], "progeny",
                             f"Unexpected domain in result: {e['domain']}")

        # Only marriage domain
        evals_marriage = self.engine.evaluate_doubtful_timing(ctx, domain="marriage")
...
class TestConfidenceModifier(unittest.TestCase):
    ...
    def test_boost_upgrades_confidence(self):
        ...
        natal = {"Venus": 4, "Saturn": 2}
        # Venus moves to exaltation (H12) → RESOLVED
        annual = {"Venus": 12, "Saturn": 3}
        ctx = MockContext(natal, annual, age=26)

        result = self.engine.get_timing_confidence(ctx, domain="marriage")
        self.assertIn(
    ...
    def test_suppress_does_not_exceed_baseline(self):
        ...
        annual = {"Rahu": 9}  # debilitated, and dormant (no other planets)
        ctx = MockContext(natal, annual, age=20)

        result = self.engine.get_timing_confidence(ctx, domain="progeny")
    ...
    def test_result_always_has_doubtful_fields(self):
        ...
        natal  = {"Sun": 1, "Moon": 2, "Venus": 7}
        annual = {"Sun": 7, "Moon": 9, "Venus": 1}
        ctx = MockContext(natal, annual, age=30)
