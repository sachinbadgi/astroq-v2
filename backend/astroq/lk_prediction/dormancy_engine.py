...
class DormancyEngine:
    ...
    def is_awake(self, planet: str, house: int, ppos: Dict[str, int]) -> bool:
        """
        Determines if a planet is active for a specific annual chart state.
        Returns True if awake, False if dormant.
        """
        state = self.get_complex_state(planet, house, ppos)
        return state["is_awake"]

    def get_complex_state(self, planet: str, house: int, ppos: Dict[str, int]) -> Dict[str, Any]:
        """
        Calculates the full activation state, including sustenance and dynamic triggers.
        Returns a dict with: is_awake, wake_reason, sustenance_factor, is_startled.
        """
        if not house:
            return {"is_awake": False, "wake_reason": "None", "sustenance_factor": 0.0, "is_startled": False}
