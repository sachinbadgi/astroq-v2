...
4. [Module 0: Chart Generator](#4-module-0-chart-generator)
5. [Module 1: Centralized Config](#5-module-1-centralized-config)
6. [Module 2: Strength Engine](#6-module-2-strength-engine)
7. [Module 3: Grammar Analyser](#7-module-3-grammar-analyser)
8. [Module 4: Probability Engine](#8-module-4-probability-engine)
9. [Module 5: Deterministic Rules Engine](#9-module-5-deterministic-rules-engine)
10. [Module 6: Event Classifier](#10-module-6-event-classifier)
11. [Module 7: Prediction Translator](#11-module-7-prediction-translator)
...
graph TD
    ...
    subgraph "Module 0: Chart Generator"
        CG["ChartGenerator<br/>DOB/TOB/POB → VedicAstro KP/Vedic → ChartData"]
    end
...
## 4. Module 0: Chart Generator

### Responsibility
Provide the foundation by calculating planetary positions (degrees, signs, nakshatras) and mapping them to Lal Kitab houses. It also constructs the 75-year Varshaphal (Annual) charts to feed into the prediction pipeline.

### Core Logic
1. **Astronomical Calculation**: Wraps `vedicastro` library to process Date, Time, and Place of Birth (DOB, TOB, POB).
2. **Chart System Selection**:
   - `kp`: Krishnamurti ayanamsa + Placidus houses
   - `vedic`: Lahiri ayanamsa + Whole Sign houses
3. **Natal Base Chart**: Extracts the 9 standard planets (Sun, Moon, Mars, Mercury, Jupiter, Venus, Saturn, Rahu, Ketu) and their house positions to form the `ChartData` dictionary.
4. **Varshaphal Generation**: Calculates 75 subsequent annual charts using Lal Kitab progression rules (the 35-year cycle, Muntha, and annual shifting).

### Inputs
- `DOB, TOB, POB`, Geographical coordinates (`lat`, `lon`), Timezone/UTC offset.
- `chart_system` string ("kp" or "vedic").

### Outputs
- List/Dict of `ChartData` objects containing `planets_in_houses` and empty state tracking logic.

---

## 5. Module 1: Centralized Config
...
| 9 | **Masnui Graha** | Two planets in same house match 13 specific combinations (e.g. Sun+Venus=Jupiter, Sun+Saturn=Mars_Bad, etc) | Masnui gets full aspects+strength; `parent_strength += cfg.masnui_parent_feedback * masnui_strength` |
| 10 | **Dhoka Graha** | Planet matches 4-Types: Age-sequence base, Birth H10, Annual H10 derived from specific Birth pairings, Annual H10 | `strength *= cfg.dhoka_graha_factor` (default: 0.70) |
| 11 | **Achanak Chot** | Pairs in Birth Houses `{1,3}`, `{2,4}`, `{4,6}`, `{5,7}`, `{7,9}`, `{8,10}`, `{10,12}`, `{1,11}` mutually aspect 100/50/25% in Annual Chart | `strength -= cfg.achanak_chot_penalty` (default: 2.00) |
